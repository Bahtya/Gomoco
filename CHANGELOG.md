...
### 新增功能
- ✨ **SFTP 协议支持**: 支持基于 SSH 的安全文件传输
- ✨ **自动生成 SSH 密钥**: SFTP 服务器自动生成 RSA 主机密钥
- ✨ **SFTP 认证**: 支持密码认证和自定义用户名
- ✨ **文件管理复用**: SFTP 复用 FTP 的 Web 文件管理 API

### 技术改进
- 🔧 使用 pkg/sftp 和 golang.org/x/crypto/ssh 实现 SFTP 服务器
- 🔧 文件管理 API 同时支持 FTP 和 SFTP
- 🔧 前端界面支持 SFTP 配置
- 🔧 模型添加 SFTP 相关字段
- 🔧 自动创建 SFTP 根目录和密钥目录
